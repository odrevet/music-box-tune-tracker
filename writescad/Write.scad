...
module writecylinder(text,where,radius,height,bold=bold,center=center,h=h,t=t,space=space,rotate=rotate,font=font,face=face,rounded=rounded,east=east,west=west,up=up,down=down,middle=middle,ccw=ccw){
	wid=(.125* h *5.5 * space);
...
module writecircle(text,where,radius,bold=bold,h=h,t=t,space=space,font=font,rotate=rotate,east=east,west=west,middle=middle,ccw=ccw){
	wid=(.125* h *5.5 * space);
	widall=wid*(len(text)-1)/2;
	//angle that measures width of letters on sphere
	function NAngle(radius)=(wid/(pi2*radius))*360;
...
module writethecylinder(text,where,radius,height,r1,r2,bold=bold,center=center,h=h,t=t,space=space,rotate=rotate,font=font,face=face,rounded=rounded,up=up,down=down,east=east,west=west){
	wid=(.125* h *5.5 * space);
	widall=wid*(len(text)-1)/2; 
	//angle that measures width of letters on sphere
...
module writesphere(text,where,radius,bold=bold,h=h,t=t,space=space,rotate=rotate,center=center,font=font,spin=spin,north=north,east=east){
	wid=(.125* h *5.5 * space);
	widall=wid*(len(text)-1)/2;
...
module writecube(text,where,size,bold=bold,h=h,t=t,rotate=rotate,space=space,font=font,face=face,up=up,down=down,left=left,right=right){
	if (str(size)[0] != "["){  
...
module writethecube(text,where,size,bold=bold,h=h,t=t,rotate=rotate,space=space,font=font,face=face,up=up,down=down,left=left,right=right){
		if (face=="front") {
...
module write(word,center=center,bold=bold,h=h,t=t,space=space,center=center,rotate=rotate,font=font){
